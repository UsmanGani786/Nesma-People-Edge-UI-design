	...
	function renderSection(position) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				if (i > 0) {
					e.append("<sclass='fc-header-space'/>");
				}
				var prevButton;
...
function View(element, calendar, viewName) {
	...
	//
	// 3. Convert the "day offset" into a "date" (a Javas Date object).
	//
...
$( "#modal-dialog" ).on('shown.bs.modal', function(){      
    ...
    $('#calendar').fullCalendar('render'); // this is needed if the calendar is not immediately visible on DOM ready
    });  
